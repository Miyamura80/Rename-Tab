...
// Fails if an em dash (U+2014) appears anywhere in the repo's text files.
// An em dash is a reliable "AI-written prose" tell; banning it keeps the
// docs and code comments in a human register.

const REPO_ROOT = join(import.meta.dir, "..");
const EM_DASH = "\u2014";

// Skipped only at the repo root.
const ROOT_SKIP_DIRS = new Set([".git", ".cache", "coverage", "out"]);

// Skipped at any depth (build output, deps).
const RECURSIVE_SKIP_DIRS = new Set(["node_modules", "dist", "build", ".next"]);
...
function shouldSkipDir(name: string, parts: string[]): boolean {
    ...
    return RECURSIVE_SKIP_DIRS.has(name);
}
...
async function* iterTextFiles(root: string): AsyncGenerator<string> {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (!shouldSkipDir(entry.name, parts)) yield* iterTextFiles(fullPath);
        } else if (entry.isFile()) {
            if (!SKIP_SUFFIXES.has(extname(entry.name).toLowerCase())) yield fullPath;
        }
    }
}
...
function findEmDashes(text: string): Array<{ lineno: number; line: string }> {
    ...
    for (const [i, line] of text.split("\n").entries()) {
        if (line.includes(EM_DASH)) results.push({ lineno: i + 1, line: line.trim() });
    }
    return results;
}

async function main(): Promise<number> {
    const violations: Array<{ relPath: string; lineno: number; snippet: string }> = [];
